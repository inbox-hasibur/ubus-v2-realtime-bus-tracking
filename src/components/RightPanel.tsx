...
export default function RightPanel({ isOpen, currentLocation }: { isOpen: boolean, currentLocation?: string | null }) {
  return (
    <div className={`absolute top-16 right-4 left-4 md:left-auto md:top-24 md:right-6 w-auto md:w-[320px] space-y-3 md:space-y-4 z-10 transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) ${isOpen ? 'translate-x-0 opacity-100' : 'translate-x-[120%] opacity-0 pointer-events-none'}`}>
      ...
      <PanelCard title="Travel Information">
         <div className="space-y-3">
           <InfoBlock label="Current location" value={currentLocation ?? 'Unknown'} />
     ...
     <div className="relative group">
       ...
       <div className="relative">
          <input type="text" placeholder={currentLocation ?? "Your location..."} className="w-full bg-slate-50 text-slate-900 text-xs font-bold p-4 pr-10 rounded-2xl outline-none placeholder:text-slate-400 border border-slate-200 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10 transition-all" />
